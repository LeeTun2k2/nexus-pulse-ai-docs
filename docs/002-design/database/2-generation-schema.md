...
CREATE TABLE generation_requests (
    ...
    user_prompt      TEXT NOT NULL,                -- User's input prompt (User input)
    
    -- User inputs
    user_media_urls TEXT[],                        -- Array of user uploaded media URLs (images, videos, etc.) (Optional) (Các media user đính kèm vào prompts)
    input_metadata   JSONB,                        -- Additional input metadata (e.g., {"product_url": "...", "batch_size": 10}) (Optinal) (Các config đặt biệt, để dễ mở rộng)
...
**MVP**:
- **Bắt buộc**: `id`, `request_id`, `user_id`, `feature_id`, `category`, `status`, `input_json`, `created_at`, `updated_at`.
- **Optional**: `project_id`, `output_metadata`, `error_message`, `error_code`, `started_at`, `completed_at`.
...
- **Optional**: `job_id`, `file_size`, `duration`, `metadata`.
...
erDiagram
    ...
    GENERATION_REQUESTS {
        ...
        jsonb input_metadata "Input metadata"
...
4. System tạo `generation_requests` với:
   - `template_prompt`: từ template (nếu có)
   - `system_prompt`: từ feature
   - `user_prompt`: từ user input
   - `user_media_urls`: từ uploads
5. System check quota/credits (Billing domain).
6. System tạo `generation_jobs` với status `pending`.
...
- **Workspace Domain**:
  - `generation_requests` có thể reference `projects.id` (sẽ tạo schema sau).
  - Projects là container cho nhiều requests.
